...
for(num_k_idx in 1:4)
{
  ...
  dkw_quant_val1 = dkw_quant_one_side(delta = delta_n,
                                      ...
                                      type = "Array" )
  ############ 
  temp_df = expand.grid(Method = c("DKW","DW","Union"), alpha = alpha_preset)
  temp_df = temp_df %>% mutate(alpha.corr = NA)
  
  for( i in 1:length(alpha_preset))
  {
    alpha_n = alpha_preset[i]
    temp_df$alpha.corr[(i-1)*3 + 1  ] = alpha_dkw_one_side(alpha = alpha_n,
                                                           I = alpha_preset,
                                                           n = n_calib,
                                                           dkw_quant_val = dkw_quant_val1,
                                                           type = "Array")
    
    temp_df$alpha.corr[(i-1)*3 + 2  ] = alpha_dw_one_side(alpha = alpha_n,
                                                          I = alpha_preset,
                                                          n = n_calib,
                                                          dw_quant_val = dw_quant_val1,
                                                          type = "Array")
    
    temp_df$alpha.corr[(i-1)*3 + 3 ] = 1 - (qbinom(1-(delta_n/length(alpha_preset)), 
  ...
  # ggsave(paste("plots_compare_union_alphas_5000/tail",num_k_idx,"_5000.pdf", sep = ""),
...
plot1_comb = annotate_figure(plot1_comb,
                             top = text_grob(expression(paste("Comparing union bd vs. simul, [0.1,0.2], 
                                                              n = 5000, ",
                                                              delta," = 0.1" ,sep = "")), 
                                             size = 12),
...
plot2_comb = annotate_figure(plot2_comb,
                             top = text_grob(expression(paste("Comparing union bd vs. simul, [0.45,0.55],
                                                              n = 5000, ",
                                                              delta," = 0.1" ,sep = "")), 
                                             size = 12),
                             left = text_grob("alpha - alpha.corr", 
...
plot3_comb = annotate_figure(plot3_comb,
                             ...
                             left = text_grob("alpha - alpha.corr", 
                                              rot = 90))
plot3_comb
ggsave("plot_far_5000.pdf",
       width = 6, height = 6, dpi = 300, units = "in" )

##########

# plot_all = ggarrange(
#   plot1[[1]] + ylim(c(-0.03,0)), plot1[[2]] + ylim(c(-0.03,0)), plot1[[3]] + ylim(c(-0.03,0)), 
#   plot2[[1]] + ylim(c(-0.03,0)), plot2[[2]] + ylim(c(-0.03,0)), plot2[[3]] + ylim(c(-0.03,0)), 
#   plot3[[1]] + ylim(c(-0.03,0)), plot3[[2]] + ylim(c(-0.03,0)), plot3[[3]] + ylim(c(-0.03,0))
# )
# 
# 
# 
...
plot_all = ggarrange(
  plot1[[1]] + ylim(c(-0.03,0)) + ggtitle("[0.1,0.2]"), 
  plot2[[1]] + ylim(c(-0.03,0)) + ggtitle("[0.45,0.55]"), 
  plot3[[1]] + ylim(c(-0.03,0)) + ggtitle("[0.02,0.8]"),
  plot1[[2]] + ylim(c(-0.03,0)), 
  plot2[[2]] + ylim(c(-0.03,0)),
  plot3[[2]] + ylim(c(-0.03,0)),
  plot1[[3]] + ylim(c(-0.03,0)), 
  plot2[[3]] + ylim(c(-0.03,0)), 
  plot3[[3]] + ylim(c(-0.03,0)),
  common.legend = TRUE, legend="bottom"
)
...
ggsave("plot_all_one_side_5000.pdf",
