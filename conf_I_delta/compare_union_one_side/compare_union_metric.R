...
for(num_k_idx in 1:4)
{
  ...
  dkw_quant_val1 = dkw_quant_one_side(delta = delta_n,
                                      ...
                                      type = "Array" )
  ############ 
  temp_df = expand.grid(Distance = c("DKW","DW"), Method = c("Simul", "Union"),
                        alpha = alpha_preset)
  ...
  p_temp = temp_df %>%
    ggplot(mapping = aes(x = alpha, y = alpha.corr - alpha , color = Distance,
                         linetype = Method)) +
...
plot1_comb = annotate_figure(plot1_comb,
                             ...
                             left = text_grob("alpha - alpha.corr", 
                                              rot = 90))
plot1_comb
ggsave("plot_tail_5000_metric_bw.pdf",
...
plot3_comb = annotate_figure(plot3_comb,
                             top = text_grob(expression(paste("Comparing union bd vs. simul, [0.02,0.8], n = 1000, ",
                                                              delta," = 0.1" ,sep = "")), 
                                             size = 12),
                             left = text_grob("alpha - alpha.corr", 
                                              rot = 90))
...
plot_all = ggarrange(
  plot1[[1]] + ylim(c(-0.05,0)) + ggtitle("[0.1,0.2]"), 
  plot2[[1]] + ylim(c(-0.05,0)) + ggtitle("[0.45,0.55]"), 
  plot3[[1]] + ylim(c(-0.05,0)) + ggtitle("[0.02,0.8]"),
  plot1[[2]] + ylim(c(-0.05,0)), 
  plot2[[2]] + ylim(c(-0.05,0)),
  plot3[[2]] + ylim(c(-0.05,0)),
  plot1[[3]] + ylim(c(-0.05,0)), 
  plot2[[3]] + ylim(c(-0.05,0)), 
  plot3[[3]] + ylim(c(-0.05,0)),
  common.legend = TRUE, legend="bottom"
)
...
# ggsave("plot_all_one_side_1000_metric.pdf",
